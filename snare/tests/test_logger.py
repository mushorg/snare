...
class TestLogger(unittest.TestCase):
    ...
    def test_create_clone_logger(self):
        self.assertIsNone(Logger.create_clone_logger(
            self.cloner_log_file, __name__))

    def test_create_logger(self):
        self.assertIsInstance(self.logger, logging.Logger)

    def test_filter(self):
        self.assertTrue(LevelFilter(logging.ERROR).filter(
            logging.makeLogRecord(self.record_dict)))
